...
if [ "$#" -ne 3 ]; then
    echo "用法: $0 <探针1> <探针2> \"函数1 函数2 ...\""
    echo "示例: $0 'ksm1=follow_page_pte:4' 'ksm2=follow_page_pte:19' \"cmp_and_merge_page follow_pmd_mask\""
    exit 1
fi
...
# 录制
# -------------------------------------------------------
echo "[3/5] 开始录制 (全系统监控, 4秒)..."
# 注意：使用 -a 进行全系统抓取，-g 抓取调用栈
perf record -a -g -e "$EVENT_START,$EVENT_END" -- sleep 4 > /dev/null 2>&1
