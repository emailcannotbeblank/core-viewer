...
const handleAnalyze = async () => {
  ...
  try {
    ...
    // 核心改造：使用原生 fetch 进行流式(Streaming)请求
    ...
    // 循环读取后端的每一次 yield 吐出的数据块
    while (true) {
      const { done, value } = await reader.read()
      if (done) break // 后端脚本执行完毕，退出循环
      
      // 解码并追加到输出框
      const chunkText = decoder.decode(value, { stream: true })
      outputLog.value += chunkText
      await scrollToBottom() // 每次接收新数据都滚到底部
    }
...
<style>
/* 样式保留你的原版设定，只加一个 input-row 来横向排列 */
