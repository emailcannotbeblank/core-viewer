...
const formData = ref({ functionName: '', callStack: '' })
const outputLog = ref('')

const selectTab = (idx) => currentTab.value = idx

const handleAnalyze = () => {
  if(!formData.value.functionName) return alert('请输入函数名')
  outputLog.value = `[${new Date().toLocaleTimeString()}] 指令已发送: ${formData.value.functionName}\n等待后端响应...\n----------------\n(模拟数据)`
}
